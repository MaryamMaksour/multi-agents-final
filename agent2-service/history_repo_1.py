...
CREATE INDEX IF NOT EXISTS idx_history2_session_created
ON  history2(session_id, created_at);

CREATE INDEX IF NOT EXISTS idx_history2_session_turn
ON  history2(session_id, turn_id);

CREATE INDEX IF NOT EXISTS idx_history2_event_type
ON  history2(event_type);
"""
...
async def ensure_history_schema() -> None:
    ...
    logger.info(" history2 schema ensured (table:  history2).")
...
async def _insert_event(
    ...
    if event_type == "user":
        ...
        sql = """
        INSERT INTO  history2 (session_id, turn_id, event_type, payload,embed_user_query )
        ...
        sql = """
        INSERT INTO  history2 (session_id, turn_id, event_type, payload,time )
        ...
        sql = """
        INSERT INTO  history2 (session_id, turn_id, event_type, payload )
...
async def log_pipeline(
    ...
    """
    Stores ONE row in ` history2` for the entire pipeline of a turn.
...
async def log_user_message(
    ...
    """
    Stores a user message event. One row in ` history2`.
    """
    payload = {
...
async def log_assistant_final(
    ...
    """
    Stores the final assistant answer event. One row in ` history2`.
...
async def get_session_history(
    ...
    """
    Returns raw  history2 rows as JSON-safe dicts.
    """
    pool = await get_pool()
...
async def delete_session_history(session_id: str) -> None:
    """
    Deletes all  history2 rows for a session.
    """
    pool = await get_pool()
    sql = "DELETE FROM  history2 WHERE session_id = $1"
    async with pool.acquire() as conn:
