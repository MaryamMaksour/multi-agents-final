...
async def _semantic_table_records(
    ...
    if table.lower() not in domain[2]:
...
async def get_table_records(query: str, table_name: str, mx: int = 5) -> Dict[str, Any]:
    ...
    if table not in domain[2]:
        return {"error": f"Unknown table: {table_name}. use one of the tables in the schema only {domain[2]}"}
...
async def get_filter(columns: List[str], table_name) -> List[str]:
    ...
    try:
        table_name = table_name.lower()
        if table_name not in domain[2]:
                return f"{table_name} Not part of this agent, use other tables from {domain[2]}"
        filters = []
...
async def get_table_schema(tables: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        for table in tables:
            if table.lower() not in domain[2]:
                return f" {table} Not part of this agent, use other tables from  {domain[2]}. "
