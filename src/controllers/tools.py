...
# table list to check against (helpers.static.domain[N]). This is that same
...
from helpers.config import DIST_OP
...
from stores.cache.vector_token_store import get_vector, store_vector
...
def build_domain_tools(allowed_tables: List[str], log_sql_query: Callable[..., Awaitable[None]]) -> list:
    ...
    `helpers.static.domain[N]` reference here - this takes the table list
    as a parameter instead, which is also what makes merging two domains
    into one agent a one-line change (pass the union of both table lists).
    """
