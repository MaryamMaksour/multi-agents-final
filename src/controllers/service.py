...
# extracted, except for which run_agent/history-logging functions it called
# under the hood - those are now injected at construction time.
...
from utils.pipeline_utils import extract_pipeline

from .running_agent import make_human_message, run_agent_without_history
...
class AgentService:
    ...
    def __init__(
        ...
        self.log_user_message = log_user_message
        self.log_assistant_final = log_assistant_final
    ...
    async def achat(self, session_id: str, user_input: Any, context: Optional[Dict[str, Any]] = None) -> Any:
        ...
        try:
            await self.log_user_message(session_id, turn_id, user_input, context)
        except Exception:
            pass

        start_time = time.time()
        result = await run_agent_without_history([make_human_message(envelope)], self.run_agent)
        end_time = time.time()
        ...
        try:
            await self.log_assistant_final(session_id, turn_id, steps, str(end_time - start_time))
        except Exception:
            pass
