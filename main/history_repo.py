...
from .conect_to_DB import get_pool
...
CREATE INDEX IF NOT EXISTS idx_history_session_turn
ON history(session_id, turn_id);

CREATE INDEX IF NOT EXISTS idx_history_event_type
ON history(event_type);
"""
...
async def ensure_history_schema() -> None:
    ...
    logger.info("History schema ensured (table: history).")
...
async def _insert_event(
    ...
    if event_type == "user":
        ...
        sql = """
        INSERT INTO history (session_id, turn_id, event_type, payload,embed_user_query )
        VALUES ($1, $2, $3, $4::jsonb, $5::vector)
        """
     
        async with pool.acquire() as conn:
            await conn.execute(sql, session_id, turn_uuid, event_type, _json_dumps(payload), embed_user_query)

    elif event_type == "assistant_final":

        sql = """
        INSERT INTO history (session_id, turn_id, event_type, payload,time )
        ...
        sql = """
        INSERT INTO history (session_id, turn_id, event_type, payload )
...
async def log_pipeline(
    ...
    """
    Stores ONE row in `history` for the entire pipeline of a turn.
...
async def log_user_message(
    ...
    """
    Stores a user message event. One row in `history`.
    """
    payload = {
...
async def log_assistant_final(
    ...
    """
    Stores the final assistant answer event. One row in `history`.
...
async def delete_session_history(session_id: str) -> None:
    ...
    """
    pool = await get_pool()
    sql = "DELETE FROM history WHERE session_id = $1"
    async with pool.acquire() as conn:
