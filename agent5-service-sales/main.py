...
from tasks.agent_chat import run_sales_chat
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Booking Sub‑agent execution failed: {str(e)}"
        )
...
async def chat_async(request: ChatRequest):
    ...
    task = run_sales_chat.delay(request.session_id, request.user_input, request.context)
