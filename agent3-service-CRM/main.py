...
from tasks.agent_chat import run_crm_chat
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"CRM Sub‑agent execution failed: {str(e)}"
        )
...
async def chat_async(request: ChatRequest):
    ...
    task = run_crm_chat.delay(request.session_id, request.user_input, request.context)
