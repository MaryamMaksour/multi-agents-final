...
CREATE TABLE IF NOT EXISTS history4 (
...
CREATE INDEX IF NOT EXISTS idx_history4_session_created
ON  history4(session_id, created_at);

CREATE INDEX IF NOT EXISTS idx_history4_session_turn
ON  history4(session_id, turn_id);

CREATE INDEX IF NOT EXISTS idx_history4_event_type
ON  history4(event_type);
"""
...
async def ensure_history_schema() -> None:
    ...
    logger.info(" history4 schema ensured (table:  history4).")
...
async def _insert_event(
    ...
    if event_type == "user":
        ...
        sql = """
        INSERT INTO  history4 (session_id, turn_id, event_type, payload,embed_user_query )
        ...
        sql = """
        INSERT INTO  history4 (session_id, turn_id, event_type, payload,time )
        ...
        sql = """
        INSERT INTO  history4 (session_id, turn_id, event_type, payload )
...
async def log_pipeline(
    ...
    """
    Stores ONE row in ` history4` for the entire pipeline of a turn.
...
async def log_user_message(
    ...
    """
    Stores a user message event. One row in ` history4`.
    """
    payload = {
...
async def log_assistant_final(
    ...
    """
    Stores the final assistant answer event. One row in ` history4`.
...
async def get_session_history(
    ...
    """
    Returns raw  history4 rows as JSON-safe dicts.
    """
    pool = await get_pool()
...
async def delete_session_history(session_id: str) -> None:
    """
    Deletes all  history4 rows for a session.
    """
    pool = await get_pool()
    sql = "DELETE FROM  history4 WHERE session_id = $1"
    async with pool.acquire() as conn:
